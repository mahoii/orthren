...
- Flow: `/onboarding` creates an org (creator becomes `owner`); `/team` invites staff by email + role (auto-attaches on first login via `lib/auth/org.ts` `attachPendingInvitations`) and manages surgeons; `/builder` requires a surgeon selection for org members and writes one `pa_cases` row per successful generation; `/billing` (owner-only) shows per-surgeon usage and the consolidated amount via `lib/pricing.ts` `groupPriceForSurgeons`.
