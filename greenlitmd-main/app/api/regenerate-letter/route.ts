...
RULE 8: When documenting conservative treatments, every treatment entry must include an outcome statement. If the outcome is explicitly documented in the source data, use it verbatim or paraphrased. If the outcome is null or absent for a treatment that shares a category with another documented treatment (e.g., two corticosteroid injections where one has a documented outcome), apply the documented outcome pattern to the undocumented entry using language such as 'clinical response was consistent with the contralateral injection, providing only transient relief without durable improvement.' Never leave a treatment entry without an outcome clause.

RULE 9: When ASA classification is present in source data, include it in the surgical plan paragraph using this format: 'Pre-operative evaluation has classified this patient as ASA [Class], confirming surgical candidacy with appropriate perioperative risk stratification.'
...
The body must: (1) Establish the clinical presentation - chief complaint, duration, severity, and specific functional limitations. Apply RULE 7 (objective measurements including BMI) and RULE 8 (treatment outcomes) throughout. If objective_measurements are provided in the prompt, integrate them into the clinical presentation paragraph using precise clinical language. (2) Document conservative care chronologically - every treatment tried, how long, and why it failed. Payers require proof that surgery is a last resort. (3) Reference imaging findings using precise medical language that directly supports the surgical indication - only reference imaging explicitly documented in the chart data. (4) State the specific procedure with anatomical detail - laterality, approach, implants if applicable. Apply RULE 9 for ASA classification and RULE 10 for bilateral staging language. (5) Close with a statement of medical necessity referencing the patient's inability to maintain activities of daily living. Apply RULE 6 to this closing paragraph. The letter must end with exactly one signature block in this exact format: 'Sincerely,' on one line, then '[Provider Name], MD' on the next line, then the practice name on the next line ONLY if a non-empty practice name was provided in the request details. If practice name is empty or was not provided, omit the practice name line entirely. Never write 'Orthopedic Practice' unless that was explicitly provided as the practice name. Write in formal clinical language. Do not use bullet points - this must read as a professional medical letter.`;
